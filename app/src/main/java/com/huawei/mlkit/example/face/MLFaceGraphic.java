...
public class MLFaceGraphic extends GraphicOverlay.Graphic {
    ...
    private List<String> sortHashMap(HashMap<String, Float> map) {
        ...
        if (this.mFace.getFaceShapeList() != null) {
            for (MLFaceShape faceShape : this.mFace.getFaceShapeList()) {
                ...
                for (int i = 0; i < points.size(); i++) {
                    ...
                    canvas.drawPoint(this.translateX(point.getX()), this.translateY(point.getY()),
                            this.boxPaint);
                    if (i != (points.size() - 1)) {
                        ...
                        if (point.getX() != null && point.getY() != null) {
                            ...
                            canvas.drawLines(new float[]{this.translateX(point.getX()),
                                    this.translateY(point.getY()), this.translateX(next.getX()),
                                    this.translateY(next.getY())}, this.getPaint(faceShape));
                        }
                    }
                }
            }
